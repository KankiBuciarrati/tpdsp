...
export const trapezoidalIntegration = (y: number[], t: number[]): number => {
    let sum = 0;
...
export const simpsonsIntegration = (y: number[], t: number[]): number => {
...
export const calculateEnergyTrapeze = (
...
export const calculateEnergySimpson = (
    signalFunc: (t: number[]) => number[],
    tStart: number = -10,
    tEnd: number = 10,
    numPoints: number = 10001
): number => {
...
export const analyzeAllSignals = (
    ...
    results.sort((a, b) => {
        if (!isFinite(a.energy) && !isFinite(b.energy)) return 0;
        if (!isFinite(a.energy)) return 1;
        if (!isFinite(b.energy)) return -1;
